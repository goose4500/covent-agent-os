...
**GitHub PR convenience tools** (`extensions/github-pr-tools.ts`): the runner registers `github_get_pr`, `github_pr_comment`, `github_create_pr`, and `github_merge_pr` as top-level Pi tools so the model can drive PR work directly without the `mcp({ server: "github", tool: "…" })` proxy round-trip. The mutating `github_create_pr` and `github_merge_pr` tools own the Slack approval card themselves (via `ctx.ui.confirmWithPreview`) — the human always sees a structured preview before GitHub is hit, and a rejection or timeout returns `isError` without calling the API. They use the same `GITHUB_MCP_PAT` env var as the MCP server and default to `goose4500/covent-agent-os`. The MCP proxy remains available for surface area outside these four actions.
