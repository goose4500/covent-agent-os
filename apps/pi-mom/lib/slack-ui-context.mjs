...
export function createSlackUIContext({
  ...
  // thread, optionally followed by a Block Kit actions row. The actions live
  // on a separate `chat.postMessage` because Slack ignores `blocks` on
  // `files.completeUploadExternal` when `initial_comment` is set; the
  // follow-up also survives Slack's file unfurl and remains independently
  // editable. Fail-soft mirrors `notify` — on error we trace and return
  // `{ ok: false, error }` without throwing into the Pi tool.
  async function postFile(filename, filePath, mimeType, opts = {}) {
    ...
    const description = opts.description ? String(opts.description).slice(0, 1000) : `📎 ${safeName}`;
    try {
      ...
      let followup = null;
      const regen = opts.regeneratePrompt ? String(opts.regeneratePrompt).slice(0, 1900) : "";
      if (regen) {
        followup = await client.chat.postMessage({
          channel,
          thread_ts: threadTs,
          text: `Actions for ${safeName}`,
          blocks: [{
            type: "actions",
            block_id: `pi_artifact_${requestId}_${Date.now().toString(36)}`.slice(0, 255),
            elements: [{
              type: "button",
              action_id: "pi_run_regenerate",
      ...
      trace("slack_ui.postFile", { requestId, filename: safeName, mimeType: mimeType || null, hasFollowup: !!followup });
