...
// Map a mime type or filename extension to a short, human label for the Card
// subtitle. Keeps the table tiny — anything not listed falls back to the
// uppercased extension, then the raw mime type, then "File".
const MIME_LABELS = {
  "text/csv": "CSV",
...
function formatLabelFor(mimeType, filename) {
  if (mimeType) {
    const key = String(mimeType).toLowerCase().split(";")[0].trim();
...
export function createSlackUIContext({
  ...
  // Upload a file artifact (generated by a Pi tool) into the current Slack
  // thread and follow it with a Block Kit Card block (Slack Apr-2026) carrying
  // filename / format / size / description, plus a sibling Context block with
  // the request id and timestamp. The Card lives on a separate
  // `chat.postMessage` because Slack ignores `blocks` on
  // `files.completeUploadExternal` when `initial_comment` is set, and we want
  // the bordered Card visual rather than the inline-comment treatment. Fail-
  // soft mirrors `notify` — on error we trace and return `{ ok: false, error }`
  // without throwing into the Pi tool.
  async function postFile(filename, filePath, mimeType, opts = {}) {
    if (disposed) return { ok: false, error: "ui_disposed" };
    const safeName = String(filename || "artifact").slice(0, 200);
    try {
      const [stat, upload] = await Promise.all([
        statFile(filePath),
      ...
      const formatLabel = formatLabelFor(mimeType, safeName);
      const subtitleParts = [formatLabel, sizeLabel].filter(Boolean);
      const cardBlock = {
        type: "card",
        title: { type: "mrkdwn", text: `*${safeName}*`.slice(0, 150) },
      };
      if (subtitleParts.length) {
        cardBlock.subtitle = { type: "mrkdwn", text: subtitleParts.join(" · ").slice(0, 150) };
      }
      if (opts.description) {
        cardBlock.body = { type: "mrkdwn", text: String(opts.description).slice(0, 200) };
      }
      const contextBlock = {
        type: "context",
        elements: [{
          type: "mrkdwn",
          text: `Generated by Pi · \`${requestId}\` · ${new Date().toISOString()}`,
        }],
      };
      const followup = await client.chat.postMessage({
        ...
        thread_ts: threadTs,
        text: `📎 ${safeName}`,
        blocks: [cardBlock, contextBlock],
      });
      trace("slack_ui.postFile", {
        requestId,
        filename: safeName,
        mimeType: mimeType || null,
        sizeBytes: stat?.size ?? null,
      });
      return { ok: true, upload, followup };
    } catch (err) {
      const error = err?.data?.error || err?.message || String(err);
      trace("slack_ui.postFile_failed", { requestId, filename: safeName, error });
