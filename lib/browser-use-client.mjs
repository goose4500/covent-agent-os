...
export function redactBrowserUseSecrets(text) {
  return String(text ?? "")
    .replace(/bu_[A-Za-z0-9._\-=]+/g, "[browser-use-api-key omitted]")
    .replace(/(apiKey=)[^&\s]+/gi, "$1[browser-use-api-key omitted]")
    .replace(/(X-Browser-Use-API-Key["':\s]+)[^"'\s,}]+/gi, "$1[browser-use-api-key omitted]");
}
